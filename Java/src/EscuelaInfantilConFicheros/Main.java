...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            // --- 5b: Matrículas del próximo mes por consola ---
            System.out.println("\nMes: " + mes + ", Año: " + anno);
        ...
        try (ObjectOutputStream oos = new ObjectOutputStream(
                new FileOutputStream("MiEscuela.escuela"))) {
            oos.writeObject(gm);
            System.out.println("\nGestionMatriculas serializada en MiEscuela.escuela");
        } catch (IOException e) {
            System.err.println("Error al serializar: " + e.getMessage());
        }

        // --- 5d: Informe anual de texto ---
        try (PrintWriter pw = new PrintWriter(new FileWriter("Informe anual de matrículas.txt"))) {
            for (int m = 1; m <= 12; m++) {
                double ingresos = gm.ingresosPorMesAnno(m, anno);
                pw.println("Mes: " + m + ", Año: " + anno + ", Ingresos: " + ingresos + "€");
                for (Nivel n : Nivel.values()) {
                    pw.println("  " + n);
                    for (Matricula mat : gm.getListaMatriculas()) {
                        if (mat.getActividad().getMes() == m
                                && mat.getActividad().getAnno() == anno
                                && mat.getActividad().getNivel() == n) {
                            pw.println("    " + mat);
                        }
                    }
                }
            }
            System.out.println("Informe escrito en 'Informe anual de matrículas.txt'");
        } catch (IOException e) {
